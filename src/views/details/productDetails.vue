...
function selectSize(size) {
  tempData.size = size;
  toast.success("Your selected Size "+size);
  updateVariant();
}

function selectColor(color) {
  tempData.color = color;
  toast.success("Your selected color "+color);
  updateVariant();
}
      ...
      <form v-on:submit.prevent="getCart(FormData, 'carts/add')">
        ...
        <div v-if="resProdcutDeteails[0].choice_options[0]" class="mt-8">
          ...
          <div class="flex flex-wrap -mb-2">
            <button type="button" v-for="item in resProdcutDeteails[0].choice_options[0].options" :key="item"
              @click="selectSize(item)"
              :class="{ 'text-white': item == tempData.size ?? 'true', 'bg-[#010412]': item == tempData.size ?? 'true' }"
              class="py-1 mb-2 mr-1 border w-11 border-[#010412] hover:text-white hover:bg-[#010412]">
              {{ item }}
            </button>
          </div>
        </div>
        ...
        <div v-if="resProdcutDeteails[0].colors[0]" class="mt-8">
          ...
          <div class="flex space-x-3">
            <button type="button" v-for="colors in resProdcutDeteails[0].colors" :key="colors"
              ...
              <div :style="{ background: colors }" class="w-6 h-6 rounded-full"></div>
            </button>
          </div>
        </div>
        ...
        <div class="mt-8">
          ...
          <div class="flex justify-center mt-4 lg:mb-0">
            ...
            <button type="submit"
              class="py-2 flex justify-center w-full text-white border border-[#010412] duration-700  bg-[#010412] hover:bg-transparent hover:text-gray-600">Order
              Now</button>
          </div>
        </div>
      </form>
